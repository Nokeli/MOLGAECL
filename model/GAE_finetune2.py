...
class GraphAutoencoder(nn.Module):
    def __init__(self, task='classification', num_layer=5, emb_dim=300, feat_dim=256, drop_ratio=0, pool='mean'):
        ...
        # DDI二分类任务
        if self.task == 'classification':
            self.pred_head = nn.Sequential(nn.Linear(self.feat_dim * 2, self.feat_dim),
                                           nn.ReLU(),
                                           nn.Dropout(p=0.5),
    ...
    def forward(self, data1, data2):
        ...
        h1 = self.x_embedding1(x1[:, 0]) + self.x_embedding2(x1[:, 1])

        for layer in range(self.num_layer):
            h1 = self.encoder[layer](h1, edge_index1, edge_attr1)
            h1 = self.batch_norms[layer](h1)

            if layer == self.num_layer - 1:
                h1 = F.dropout(h1, self.drop_ratio, training=self.training)
            else:
                h1 = F.dropout(F.relu(h1), self.drop_ratio, training=self.training)

        h1 = self.pool(h1, data1.batch)
        h1 = self.decoder1(h1)
        # drug2
        x2 = data2.x
        ...
        h2 = self.x_embedding1(x2[:, 0]) + self.x_embedding2(x2[:, 1])

        for layer in range(self.num_layer):
            h2 = self.encoder[layer](h2, edge_index2, edge_attr2)
            h2 = self.batch_norms[layer](h2)

            if layer == self.num_layer - 1:
                h2 = F.dropout(h2, self.drop_ratio, training=self.training)
            else:
                h2 = F.dropout(F.relu(h2), self.drop_ratio, training=self.training)

        h2 = self.pool(h2, data2.batch)
        h2 = self.decoder1(h2)
