...
class MoleculeDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # # 创建一个i的 DGL 图
        # graph1 = dgl.DGLGraph()
        # graph1.add_nodes(x_i.cpu().detach().numpy().shape[0])  # 添加与节点特征张量 x 的大小相匹配的节点
        # src, dst = edge_index_i.cpu().detach().numpy()
        # graph1.add_edges(src, dst)  # 添加双向边的连接关系
        # graph1.ndata['feat'] = x_i
        # graph1.edata['feat'] = edge_attr_i
        ...
        # # 创建一个j的 DGL 图
        # graph2 = dgl.DGLGraph()
        # graph2.add_nodes(x_j.cpu().detach().numpy().shape[0])  # 添加与节点特征张量 x 的大小相匹配的节点
        # src, dst = edge_index_j.cpu().detach().numpy()
        # graph2.add_edges(src, dst)  # 添加双向边的连接关系
        # graph2.ndata['feat'] = x_j
        # graph2.edata['feat'] = edge_attr_j
...
class MoleculeDatasetWrapper(object):
    ...
    def get_train_validation_data_loaders(self, train_dataset):
        ...
        # train_loader = DataLoader(train_dataset, batch_size=self.batch_size, sampler=train_sampler,
        #                           num_workers=self.num_workers, drop_last=True, collate_fn=self.custom_collate_fn())
        # valid_loader = DataLoader(train_dataset, batch_size=self.batch_size, sampler=valid_sampler,
        #                           num_workers=self.num_workers, drop_last=True, collate_fn=self.custom_collate_fn())
